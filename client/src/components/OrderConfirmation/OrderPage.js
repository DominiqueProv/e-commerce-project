...
const OrderPage = () => {
  ...
  const { shippingAddress, billingInfo } = userInfo.userInfo;

  const sendOrder = () => {
    dispatch(changeStatus("purchasing"));
    const { fname, lname, email, id } = userInfo.userInfo;
    const orders = cartIds.map((itemId) => ({
      itemId: parseInt(itemId),
      numOrdered: cartById[itemId].quantity,
    }));
    const body = {
  ...
  const lastThreeDigitsOfCardNumber = billingInfo.cardNumber.substr(
    billingInfo.cardNumber.length - 3
  );
  ...
  return (
    <Wrapper data-css="form-wrapper">
      ...
      <WrapperInfo>
        <Info>
          ...
          <FlexBox>
            ...
            <h5>
              ...
              {shippingAddress.postalCode}, {shippingAddress.province}
            </h5>
          </FlexBox>
          <FlexBox>
            <p>Phone Number:</p>
        ...
        <Info>
          ...
          <FlexBox>
            <p>Name:</p>
            <h5>
              {billingInfo.fname} {billingInfo.lname}
            </h5>
          </FlexBox>
          <FlexBox>
            <p>Address:</p>
      ...
      <WrapperButton>
        ...
        <FormButton onClick={() => sendOrder()}> confirm purchase </FormButton>
      </WrapperButton>
    </Wrapper>
  );
};

...
const Form = ({ formNumber, setFormNumber, userInfo, validateForm }) => {
  ...
  const inputPhoneNumber = (event) => {
    ...
    if (phoneNumberString) {
      ...
      const justNumberFromString = phoneNumberString
        .match(RegexNumberExtractor)
        .join("");
  ...
  const handleSubmit = () => {
    ...
    if (validationResponse) {
      ...
      setInputProblem(
        "it seems that there is a problem with: " + validationResponseString
      );
    } else {
  ...
  React.useEffect(() => {
    if (userInfo) {
      if (userInfo.shippingAddress) {
        if (userInfo.shippingAddress.fname)
          setFname(userInfo.shippingAddress.fname);
        if (userInfo.shippingAddress.lname)
          setLname(userInfo.shippingAddress.lname);
        ...
        if (userInfo.shippingAddress.province)
        ...
        if (userInfo.shippingAddress.phoneNumber)
